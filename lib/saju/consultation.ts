...
export const READING_STYLE_VERSION = 2;
...
export type Consultation = {version:1;analysisVersion:1;readingStyleVersion?:2;year:number;chapters:ConsultationChapter[]};
...
export function validateConsultation(value:unknown,validIds?:string[]):Consultation {
  ...
  if(c.readingStyleVersion!==undefined && c.readingStyleVersion!==READING_STYLE_VERSION) throw new Error("상담 문체 버전을 확인해 주세요.");
  ...
  c.chapters.forEach(ch=>(c.readingStyleVersion===READING_STYLE_VERSION?validatePlainChapter:validateChapter)(ch,ch.id,validIds));
  return c;
}
...
export function consultationPrompt(context:GeminiReadingContext,id:ChapterId) {
  ...
  return [
    ...
    "3~4절을 작성합니다. 각 절 text는 한국어 400~650자를 목표로 충분히 풉니다(최소280자·2문단). 읽는 순서: 생활 속 상황 → 이 사람의 사주에서 그렇게 읽는 구체적 이유 둘 이상 → 다르게 나타날 조건 → 확인할 행동. 계산 자료를 나열하며 시작하지 마세요. 어떤 사주에도 맞는 일반론만 있으면 다시 쓰세요. 좋은 소리만 하거나 공포를 조장하지 마세요.",
