...
export default function FortunePanel({ report, reading, timezone = "Asia/Seoul" }: {
  ...
  const [tab, setTab] = useState<"lifetime" | "annual" | "domains">("lifetime");
  ...
  return <section className="fortune-panel" aria-labelledby="fortune-title">
    ...
    <div className="fortune-tabs" aria-label="운세 주제">
      <button type="button" aria-pressed={tab === "lifetime"} onClick={() => setTab("lifetime")}>평생운</button>
      <button type="button" aria-pressed={tab === "annual"} onClick={() => setTab("annual")}>{report.year}년 · 월별운</button>
      <button type="button" aria-pressed={tab === "domains"} onClick={() => setTab("domains")}>일 · 재물 · 관계</button>
    </div>
    {tab === "lifetime" && <div className="fortune-content">
      <h3>인생 전체를 이어서 읽기</h3>
      ...
      <div className="lifetime-grid">{report.lifetime.map((stage) => <article className="lifetime-card" key={stage.label}>
        ...
        {stage.periods.map((period) => <details key={period.index}>
    ...
    {tab === "annual" && <div className="fortune-content">
      <h3>{report.year}년 {report.annual.ganji}({report.annual.korean}) 세운</h3>
    ...
    {tab === "domains" && <div className="fortune-content reading-grid">
      {report.domains.map((domain, index) => <article className="reading-card" key={domain.title}>
        <h3>{domain.title}</h3><p>{reading ? [reading.career, reading.money, reading.relationships][index] : domain.body}</p>
      </article>)}
    </div>}
