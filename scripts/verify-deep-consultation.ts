...
import { CHAPTERS, validatePlainChapter, type ConsultationChapter } from "../lib/saju/consultation";
...
async function main() {
  ...
  const target=join(tmpdir(),"saju-consultation-readable-synthetic-20260923.json");
  ...
  for(const definition of CHAPTERS) {
    ...
    if(cached) {
      try { validatePlainChapter(cached,definition.id); chapters.push(cached); console.log(JSON.stringify({id:definition.id,revalidated:true})); continue; } catch { /* Only regenerate concrete validation failures. */ }
    }
    ...
    const chapter=validatePlainChapter(body.chapter,definition.id);
    chapters.push(chapter);
    writeFileSync(target,JSON.stringify({version:1,analysisVersion:1,readingStyleVersion:2,year:2026,chapters},null,2));
