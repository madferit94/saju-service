...
import { CHAPTERS, validateChapter, type ConsultationChapter } from "../lib/saju/consultation";
...
async function main() {
  ...
  const target=join(tmpdir(),"saju-consultation-synthetic-20260923.json");
  ...
  for(const definition of CHAPTERS) {
    ...
    if(cached) {
      try { validateChapter(cached,definition.id); chapters.push(cached); console.log(JSON.stringify({id:definition.id,revalidated:true})); continue; } catch { /* Only regenerate concrete validation failures. */ }
    }
    ...
    const chapter=validateChapter(body.chapter,definition.id);
    chapters.push(chapter);
    writeFileSync(target,JSON.stringify({version:1,analysisVersion:1,year:2026,chapters},null,2));
